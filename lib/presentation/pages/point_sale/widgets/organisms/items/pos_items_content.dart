...
class PosTicketManagementApi {
  ...
  Future<PaginatedResponse<GenericListItem<Map<String, dynamic>>>> fetchPage({
    ...
    return SafeExecutor.run(
        ...
        final result = rows
            .map<GenericListItem<Map<String, dynamic>>>(
              (json) {
            ...
            final dateInvoice = DateTime.tryParse(
              header['invoice_date']?.toString() ?? '',
            ) ??
                DateTime.now();

            final dateInvoiceString =
            DateFormat('dd/MM/yyyy HH:mm').format(dateInvoice);
            ...
            final serviceType =
                meta['service_type']?.toString() ?? 'TAKEAWAY';
 ...
 IconData orderIcon=Icons.shopping_bag;
            ...
            final products = details.map<Map<String, dynamic>>(
                  (detail) {
                final quantity =
                    double.tryParse(
                ...
                final total =
                    double.tryParse(
                      detail['total']?.toString() ?? '0',
                    ) ??
                        (quantity * unitPrice);

                final productType =
                    detail['product_type']?.toString() ?? 'UNIT';
                ...
                return {
                  ...
                  // Datos traducidos
                  'productTypeName': productType == 'UNIT'
                      ? 'Unidad'
                      : productType,
            ...
            final quantity =
                (firstProduct['quantity'] as num?)?.toDouble() ?? 0;
